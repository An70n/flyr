...
public class TextlineDialogueUI : StandardDialogueUI
{
    ...
    private List<DialogueEntryRecord> records = new List<DialogueEntryRecord>();

    private List<GameObject> instantiatedMessages = new List<GameObject>();

    private int currentSceneIndex = -1;

    private PreDelaySettings npcPreDelaySettingsCopy = new PreDelaySettings();
    private PreDelaySettings pcPreDelaySettingsCopy = new PreDelaySettings();

    private bool isLoadingGame = false;
    private bool skipNextRecord = false;
    private bool isInPreDelay = false;
    ...
    private void RecordCurrentScene(Scene scene, LoadSceneMode mode)
    {
    ...
    private IEnumerator ShowResponsesAfterPreDelay(Subtitle subtitle, Response[] responses, float timeout)
    {
    ...
    private void ShowResponsesNow(Subtitle subtitle, Response[] responses, float timeout)
    {
        Tools.SetGameObjectActive(npcPreDelaySettings.preDelayIcon, false);
        Tools.SetGameObjectActive(pcPreDelaySettings.preDelayIcon, false);
    ...
    private StandardUISubtitlePanel GetTemplate(Subtitle subtitle)
    {
    ...
    private IEnumerator AddMessageWithPreDelay(float preDelay, Subtitle subtitle)
    {
        isInPreDelay = true;
    ...
    /// </summary>
    private void AddMessage(Subtitle subtitle)
    {
    ...
    private T FindUIElement<T>(Transform t, string name1, string name2) where T : MonoBehaviour
    {
    ...
    private void AddRecord(Subtitle subtitle)
    {
        if (skipNextRecord)
        {
    ...
    private bool DontLoadInThisScene()
    {
